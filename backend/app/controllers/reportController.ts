...
export const getReportStatusHandler = async (
  ...
  try {
    ...
    const timezoneSecond = Date.parse(
      new Date(req.params.reportId).toUTCString()
    );
    const today = new Date();
    today.setTime(timezoneSecond);
    const tomorrow = new Date();
...
export const getUserWeeklyReportsHandler = async (
  ...
  try {
    ...
    if (weeklyDay === 0) {
      millisecondAfter =
        Date.parse(today.toLocaleString()) - 86400000 * (weeklyDay - 1);
    } else {
      millisecondAfter =
        Date.parse(today.toLocaleString()) - 86400000 * weeklyDay;
    }
    ...
    weeklyFirstDay.setHours(-7);
    weeklyFirstDay.setMinutes(0);
    weeklyLastDay.setHours(16);
    weeklyLastDay.setMinutes(59);
    const query = {
      user: { $eq: res.locals.user._id },
