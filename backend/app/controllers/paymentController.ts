...
export const createPayHistoryHandler = async (
  ...
  try {
    ...
    let monthlyPay = 0;
    monthlyPayment.map(
      (eachPayment) => (monthlyPay += parseFloat(eachPayment.amount))
    );
    ...
    eachMonthlyPayment.map(
      (eachPayment) => (eachMonthlyPay += parseFloat(eachPayment.amount))
    );
    ...
    const timezoneSecond =
      Date.parse(new Date().toUTCString()) + 3600000 * timezoneAdding;
    today = new Date();
    ...
    const payPlanQueryDate =
      lastDay.getFullYear().toString() + lastDay.getMonth().toString();
    ...
    monthlyPayPlan.map((eachPlan: any) => {
      ...
      if (individualAmounts[0] === undefined) {
        realEachMonthlyAmounts.push({
          name: eachPlan.name,
          eachMonthlyAmount: "0",
        });
      } else {
...
export const getMonthlyPaymentHandler = async (
  ...
  try {
    ...
    const resetRealMonthlyAmounts = realMonthlyAmounts.map((eachVal: any) => {
      ...
      const timezoneSecond =
        Date.parse(new Date(eachVal.created_at).toUTCString()) +
        3600000 * timezoneAdding;
...
export const updatePaymentHistoryHandler = async (
  req: Request<
    UpdatePayHistoryInput["params"],
    {},
    UpdatePayHistoryInput["body"]
  >,
  res: Response,
  next: NextFunction
) => {
  try {
    ...
    if (!updatedPayment) {
      ...
      await findAndUpdatePayment(
        {
          created_at: { $gte: firstMoment, $lte: secondMoment },
          monthlyAmount: { $ne: null },
        },
    ...
    if (today > new Date("Decemnber 25, thisYear 00:00:00")) {
      thisYear++;
    }
    ...
    for (let eachMonth = 0; eachMonth < 12; eachMonth++) {
      ...
      const eachMonthlyAmounts: any = await findPayment(queryMonthlyEachAmount);
      const allUsers = await findAllUsers();
      if (eachMonthlyAmounts.length === 0) {
        yearlyEachMonthAmount.push(0);
        continue;
      }
      let realMonthlyAmounts: any;
      monthlyAmounts.sort((p1: any, p2: any) => {
      ...
      allUsers.map((eachUser: any, key: any) => {
        ...
        if (yearlyEachTotalAmounts[key] === undefined) {
          ...
          yearlyRealIndividualAmount =
            yearlyEachTotalAmounts[key].yearlyIndividualAmount;
        }

        yearlyRealIndividualAmount += parseFloat(
          individualAmounts[0].eachMonthlyAmount
        );

        yearlyEachTotalAmounts[key] = {
          name: eachUser.name,
          yearlyIndividualAmount: yearlyRealIndividualAmount,
        };
      });
    }
    const allUsers = await findAllUsers();
...
export const getAllMonthlyPaymentHandler = async (
  ...
  try {
    ...
    const payment = [
      ...
      monthlyPayPlan,
      allUsers,
      timezoneDate.toLocaleDateString(),
    ];
...
export const getAllPaymentHistoryHandler = async (
  ...
  try {
    const timezoneSecond = Date.parse(
      new Date(req.params.paymentId).toUTCString()
    );
    const today = new Date();
    today.setTime(timezoneSecond);
    const firstDay = new Date();
    ...
    const payment = [
      yearlyTotalAmount,
      yearlyEachMonthAmount,
      yearlyEachTotalAmounts,
      monthlyEachTotalAmounts[0],
      resetRecentPayments,
      allUsers,
    ];
