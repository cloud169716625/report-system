...
router
  .route("/payplan")
  .post(validate(createPayPlanSchema), createPayPlanHandler)
  .get(getPayPlanHandler);

router
  .route("/payplan/:paymentId")
  .patch(validate(updatePayPlanSchema), updatePaymentPlanHandler)
  .delete(validate(deletePaymentSchema), deletePayPlanHandler);

router
  .route("/payhistory")
  .post(validate(createPayHistorySchema), createPayHistoryHandler);

router
  .route("/payhistory/:paymentId")
  .patch(validate(updatePayHistorySchema), updatePaymentHistoryHandler);

router.route("/monthlypay").get(getMonthlyPaymentHandler);

router.route("/monthlyall").get(getAllMonthlyPaymentHandler);
...
router.route("/yearlyall").get(getAllYearlyPaymentHandler);
