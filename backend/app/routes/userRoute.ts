...
router
  ...
  .patch(validate(updateTimeZoneSchema), updateServerTimeZoneHandler);
router
  .route("/pwd/:userinfoId")
  .patch(validate(updatePasswordSchema), updatePasswordHandler);
router
  .route("/:userinfoId")
  .patch(validate(updateRoleSchema), updateRoleHandler)
