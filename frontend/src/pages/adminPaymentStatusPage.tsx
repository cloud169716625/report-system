...
const AdminPaymentStatusPage = () => {
  ...
  const onSubmitHandler: SubmitHandler<ICreatePayHistory> = (values) => {
    if (parseFloat(values.rate) > 0 && parseFloat(values.rate) <= 1) {
      ...
      const amountValue = (
        (rateFloat * realAmountFloat) /
        Math.pow(10, ratePointNum + realAmountPointNum)
      ).toString();
      createPayHistory({
        name: values.name,
  ...
  return (
    <Container maxWidth={false}>
      ...
      >
        Incoming Status This Month
      </h5>
      <AdminPaymentStatusIllu
        transUser={(users: any) => setUsersForSelect(users)}
      />
                ...
                <Paper style={{ boxShadow: "none" }}>
                  ...
                  >
                    {usersForSelect?.map((userForSelect: any, key: any) => {
                      return (
                        <MenuItem key={key} value={userForSelect.name}>
                          {userForSelect.name}
                        </MenuItem>
                      );
                    })}
                  ...
                  <p style={{ margin: "unset", color: "gray" }}>
                    Payment Mothod
                  </p>
                  <TextField
                    fullWidth
