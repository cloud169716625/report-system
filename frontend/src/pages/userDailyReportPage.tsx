...
const UserDailyReportPage = () => {
  ...
  useEffect(() => {
    if (new Date().getUTCDay() === 0 || new Date().getUTCDay() === 6) {
      ...
      new Date().getUTCHours() + 9 < 18 ||
      new Date().getUTCHours() + 9 > 23
    ) {
      setHeaderText("This is not report time.");
    }
  }, []);
  ...
  return (
    <Container maxWidth={false}>
      ...
      >
        Please report in a few minutes
      </h5>
      <h5
        style={{
          fontSize: "15px",
      ...
      <UserDailyReportTable
        setBtnAble={(btnAble: string) => setButtonActive(btnAble)}
      />
    </Container>
  );
};
