...
const AdminReportStatusPage = () => {
  ...
  const {
    ...
    isError,
    error,
    data: reportStatusWithUsers,
  } = useGetReportStatusQuery(selectDateValue);
  ...
  const emptyRows =
    page > 0
      ? Math.max(0, (1 + page) * rowsPerPage - reportStatusWithUsers[0]?.length)
      : 0;
  ...
  useEffect(() => {
    if (reportStatusWithUsers !== undefined) {
      ...
      reportStatusWithUsers[1].map((userValue: any) => {
        const unreportFilter = reportStatusWithUsers[0].find(
          (reportValue: any) => {
  ...
  return (
    <Container maxWidth={false}>
      ...
      >
        <span style={{ color: "brown" }}>{unreportedNames.toString()}</span>{" "}
        haven't reported this day {weekend}
      </h5>
